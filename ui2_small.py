#!/usr/bin/env python3
# NOVA V2 Small UI — graphical wrapper around jarvis_v2_small.py
# Uses JSON-mode agent optimized for small models (3B-7B, Raspberry Pi).
# Run: python ui2_small.py
...
_messages = []  # V2 Small conversation state

try:
    import jarvis_v2_small as agent
...
agent.jarvis.set_ui_emit(_jarvis_to_ui)



# Theme — Green HUD for V2 Small
...
def generate_app_icon(root):
    ...
    for i in range(50, 0, -1):
        ...
        draw.ellipse([cx-r_halo, cy-r_halo, cx+r_halo, cy+r_halo],
                     outline=(52, 211, 153, alpha), width=1)
    ...
    for lj in range(16):
        ...
        for k in range(len(pts) - 1):
            ...
            draw.line([(pts[k][0], pts[k][1]), (pts[k+1][0], pts[k+1][1])],
                      fill=(52, 211, 153, shade), width=1)

    # Three rotating arc-reactor rings (ellipses at angle)
    ring_specs = [
        (R * 0.68, 22, 3, (212, 175, 55)),
        (R * 0.82, 40, 2, (52, 211, 153)),
        (R * 0.92, 15, 2, (110, 231, 183)),
    ]
    ...
    for i in range(72):
        ...
        col_tick = (212, 175, 55, 200) if i % 18 == 0 else (52, 211, 153, 120)
    ...
    try:
        ...
        print(f"[UI-SMALL] Could not set icon: {e}")
...
class CommandPalette(tk.Toplevel):
    def __init__(self, parent, on_select):
        ...
        self.title("NOVA V2 Small — Command Interface")
...
class MemoryViewer(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("NOVA V2 Small — Memory Core")
        self.configure(bg=Theme.BG)
        ...
        mems = agent.jarvis.memory.get("memories", [])
...
class NovaUI:
    ...
    def _startup_check(self):
        if agent.jarvis.check_ollama():
            self._chat.add_system(f"NOVA V2 Small online. JSON-mode agent · {agent.CHAT_MODEL}")
            self._stat_labels["OLLAMA"].config(text=agent.CHAT_MODEL, fg=Theme.SUCCESS)
        else:
